...
| R4 | `grade`/`gradeKey` come from the Choghadiya segment covering the greatest share of the window | Ganak already classifies Choghadiya as good/neutral/bad in `panchang.ts` |
| R5 | Abhijit sets `abhijitBoost: true` but **does not** clear a block | Abhijit is auspicious, not a universal solvent; a Wednesday Abhijit is itself avoided. Recorded as a deliberate conservative choice — see §8 D2 |
