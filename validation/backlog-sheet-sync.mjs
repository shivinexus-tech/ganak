...
assert.equal(sheetRow(base.rows.get("46"))[4], "40%");
assert.equal(base.rows.get("46").quality.deliveryState, "Validated feature branch — not merged or publicly delivered");
...
assert.match(base.rows.get("46").quality.recommendedAction, /brand-theme values/);
