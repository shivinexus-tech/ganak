...
assert.equal(sheetRow(base.rows.get("46"))[4], "55%");
assert.equal(base.rows.get("46").quality.deliveryState, "Core comfort/accessibility system deployed and production-verified; broader design-system migration remains");
assert.equal(base.rows.get("46").quality.qualityRisk, "Amber");
assert.match(base.rows.get("46").quality.limitations, /Muhurat Listen/);
assert.match(base.rows.get("46").quality.bugBashStatus, /slice bug bash completed 2026-08-01/);
assert.match(base.rows.get("46").quality.recommendedAction, /only token source/);
