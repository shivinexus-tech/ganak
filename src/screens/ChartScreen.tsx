          ...
          <>
            ...
            {(() => {
              ...
              return (
                <div>
                  {topRp && (
                    <div className="rise" style={{ ...card, padding: "14px 16px", borderLeft: "4px solid #A86A12", marginBottom: 12, background: "#FBF5E7" }}>
                      <div style={{ ...T.label, color: C.gold, marginBottom: 6 }}>{hi ? "पहले पढ़ें · शासक ग्रह का सार" : "Read first · ruling-planet summary"}</div>
                      <p style={{ margin: 0, color: C.ivory, lineHeight: 1.6, fontSize: 13.5 }}>
                  ...
                  {RP.ranked && (
                    <div className="rise" style={{ ...card, padding: "12px 14px", marginBottom: 14 }}>
                      <div style={{ ...T.label, color: C.muted, marginBottom: 8 }}>{hi ? "समर्थन क्रम · कौन-सा ग्रह कितनी बार आया" : "Support ranking · how often each planet appears"}</div>
                      <div style={{ display: "grid", gap: 8 }}>
                        {RP.ranked.map((rp, idx) => (
                          <div key={rp.planet} style={{ display: "grid", gridTemplateColumns: "28px minmax(54px, 70px) 1fr", alignItems: "center", gap: 8 }}>
                            <span style={{ color: C.muted, fontSize: 12 }}>#{idx + 1}</span>
                            <span style={{ color: PLANET_COLOR[rp.planet], fontWeight: 700 }}>{hi ? (PLANET_DEVA[rp.planet] || rp.planet) : rp.planet}</span>
                            <span style={{ color: C.muted, fontSize: 12.5, lineHeight: 1.4 }}>
            ...
            {(() => {
              ...
              const Tile = ({ item, accent }) => (
                <div style={{ ...card, padding: "12px 14px", borderLeft: `3px solid ${accent}` }}>
                  ...
                  <div style={{ color: C.muted, fontSize: 11.5, marginTop: 4, lineHeight: 1.45 }}>{hi ? (SPECIAL_POINT_COPY[item.k]?.hi || "यह विशेष बिंदु कुंडली के एक सूक्ष्म जीवन-विषय को दर्शाता है।") : (SPECIAL_POINT_COPY[item.k]?.en || item.note)}</div>
                  <div style={{ color: C.muted, fontSize: 11, marginTop: 4, lineHeight: 1.45, fontStyle: "italic" }}>{hi ? (SPECIAL_POINT_COPY[item.k]?.useHi || "") : (SPECIAL_POINT_COPY[item.k]?.useEn || "")}</div>
                </div>
              );
              ...
              return (
                <div>
                  <div style={{ ...card, padding: "12px 16px", marginBottom: 12, background: "#FBF5E7", borderLeft: `4px solid ${C.gold}` }}>
                    <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.6, color: C.ivory }}>
