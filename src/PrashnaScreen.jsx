// ============================================================================
// PrashnaScreen.jsx — प्रश्न कुण्डली (Prashna / horary) for Janma Kundli
//
// SPLICE NOTES (Task 4, when kundli-app.jsx is available):
//   1. Paste the ENGINE section + PrashnaScreen into kundli-app.jsx.
//   2. Register `PrashnaScreen` in the cutBlock list in validation/build-engine.js.
//   3. Dedupe: if the app already computes sun/moon longitudes or ayanamsa,
//      swap PR_* functions for the app's equivalents and delete the copies,
//      then grep "PR_" — zero orphaned references is the done-standard.
//   4. Wire lat/lon/placeLabel from the app's location state (defaults: Delhi).
//   5. Swap TOKENS for the app's existing style constants.
// Engine identical to validation/prashna-calc.js (24/24 tests vs Drik anchors).
// No localStorage. Errors render visibly in-app (phone-only testing rule).
// ============================================================================

import { useState } from "react";

// ----------------------------------------------------------------- TOKENS
// Swap these for the app's canonical constants on splice.
const TOKENS = {
  bg: "#FBF6EC", card: "#FFFFFF", ink: "#2A2118", muted: "#8A7F6E",
  line: "#E8DFCE", gold: "#B8860B", goldSoft: "#FBF1DA",
  sindoor: "#C73E26", sindoorSoft: "#FBE7E1", amber: "#B26A00", amberSoft: "#FDF3E0",
  ctrlH: 42, radius: 11,
...
function PR_helio(key, T) {
  ...
  let Er = M * D2R;
  for (let i = 0; i < 12; i++) Er = Er - (Er - e * Math.sin(Er) - M * D2R) / (1 - e * Math.cos(Er));
...
function PR_significations(chart, key) {
  ...
  const owned = g => {
    const hs = [];
    for (let h = 1; h <= 12; h++)
      if (SIGN_LORD[Math.floor(chart.cusps[h] / 30)] === g) hs.push(h);
    return hs;
  };
...
function buildReasons(v) {
  const lines = [];
  const m = HOUSE_MEANING[v.q.cusp];
  lines.push({ tone: 'lead',
    text: `${GRAHA_HI[v.cuspSub]} (${GRAHA_EN[v.cuspSub]}) holds the deciding vote here — it is the sub-lord of your ${v.q.cusp}th house, the house of ${m}.` });
  for (const h of v.hits.favor)
    lines.push({ tone: 'good', text: `It connects to your ${h}th house — ${HOUSE_MEANING[h]} — a supportive signal.` });
  for (const h of v.hits.deny)
    lines.push({ tone: 'bad', text: `It also touches your ${h}th house — ${HOUSE_MEANING[h]} — which works against this matter.` });
  if (v.retroDrag)
    lines.push({ tone: 'bad', text: `${GRAHA_EN[v.cuspSub]} is retrograde — moving backward in the sky. Expect delay, rework, or a second attempt.` });
...
// ------------------------------------------------------------ MAIN SCREEN
export default function PrashnaScreen({ lat = 28.6139, lon = 77.209, placeLabel = 'New Delhi' }) {
  ...
  return (
    <div style={{ background: TOKENS.bg, minHeight: '100%', padding: 16, color: TOKENS.ink,
      ...
      <SecHead hi="प्रश्न कुण्डली" en="Prashna · ask the moment" />
      <Gloss>
        Ask a question now — the sky at this very moment answers it. This is Prashna
        (horary astrology): no birth details needed, only the moment and place of asking.
      </Gloss>
        ...
        {QUESTIONS.map(q => {
          ...
          return (
            <button key={q.key} onClick={() => setSelected(q.key)}
              ...
              <span style={{ color: TOKENS.muted, fontSize: 12 }}> · {q.en}</span>
            </button>
          );
        })}
      </div>
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <div style={{ background: TOKENS.card, borderRadius: TOKENS.radius,
            ...
            <div style={{ background: vs.soft, padding: '14px 16px' }}>
              <div style={{ fontFamily: TOKENS.devanagari, fontSize: 28, color: vs.color, lineHeight: 1.1 }}>{vs.hi}</div>
              <div style={{ fontSize: 12, letterSpacing: '0.12em', textTransform: 'uppercase', color: vs.color }}>{vs.en}</div>
              <div style={{ fontSize: 13, color: TOKENS.muted, marginTop: 6 }}>
                {v.q.hi} · {v.q.en}
              </div>
            </div>
            <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {buildReasons(v).map((r, i) => (
              ...
              <Gloss>
                Cast for {result.askedAt.toLocaleString()} at {placeLabel}. Prashna is judged
                for the moment you ask, at the place you ask from.
              </Gloss>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
            <Chip label="लग्न · Lagna" value={`${RASHI_HI[result.chart.lagna.sign]} ${fmtDeg(result.chart.lagna.deg)}`}
              gloss="the sign rising in the east at this moment" />
            <Chip label="नक्षत्र · Nakshatra" value={`${result.chart.lagna.nak.en}-${result.chart.lagna.nak.pada}`}
              gloss="the lunar mansion the rising degree falls in" />
            <Chip label="उप-स्वामी · Sub-lord" value={GRAHA_EN[v.cuspSub]}
              gloss="the fine-grained ruler that casts the deciding vote" />
          </div>
          ...
          <button onClick={() => setShowFull(s => !s)}
            ...
            {showFull ? 'विवरण छिपाएँ · Hide details' : 'विस्तृत प्रश्न कुण्डली · Full Prashna chart'}
          </button>
          ...
          {showFull && (
            <div style={{ marginTop: 10, background: TOKENS.card, borderRadius: TOKENS.radius,
                ...
                <tbody>
                  {result.chart.planets.map(p => (
                    <tr key={p.key} style={{ borderTop: `1px solid ${TOKENS.line}` }}>
                      ...
                      <td>{RASHI_EN[p.sign]} {fmtDeg(p.deg)}</td>
                      <td>{p.nak.en}-{p.nak.pada}</td>
                      <td>{p.star}/{p.sub}</td>
                      <td>{p.house}</td>
                    </tr>
                  ))}
              ...
              <div style={{ marginTop: 10 }}>
                <Gloss>
                  Star/Sub = nakshatra lord / KP sub-lord — the two-level rulership Prashna
                  reads. H = house occupied ({result.chart.system === 'placidus'
                    ? 'Placidus cusps, the KP standard'
                    : 'equal houses — high-latitude fallback'}). Positions: Lahiri ayanamsa,
                  mean Rahu/Ketu — the same conventions as Drik Panchang defaults.
                </Gloss>
              </div>
            </div>
          )}
...
function Chip({ label, value, gloss }) {
  return (
