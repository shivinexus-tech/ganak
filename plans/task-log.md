...
| CODEX-P1-BUNDLE | PARTIAL | Codex | `main` | see `plans/codex-task-phase1-bundle.md` | Tracks 1–2 MERGED; Track 3 hide local / deploy BLOCKED | See `P1-HIDE-DEPLOY`. Reassigned: Chart panels under SHELL-FINISH-48H. |
...
| P1-HIDE-DEPLOY | ACTIVE | Codex | `main` | hosting config, `plans/deploy-notes.md` | Deploy the already-hidden Phase 1 web build; do not edit shell | Owner reactivated deployment 2026-07-19 after Cursor merged ChartScreen (`06378e7`). CLI deployment in progress; `src/kundli-app.tsx` explicitly out of scope. |
...
| CLAUDE-LAUNCH-PRIVACY | RESERVED | Claude Code | `main` | `src/kundli-app.tsx`, `public/fonts/**` (or equiv), font CSS, `plans/legal-privacy-terms-draft.md`, `plans/claude-task-launch-privacy.md`, `plans/task-log.md`; may touch `server/**` if smoke fails | Fix false footer; self-host Eczar+Spectral; run `server` smoke suite | Brief: `plans/claude-task-launch-privacy.md`. Stay off MuhuratHub. |
...
| CODEX-NEXT | RESERVED | Codex | `main` | see notes | **Priority:** finish `P1-HIDE-DEPLOY` (commit hide if needed + deploy). Parallel OK: Chart panel peels OR more P1-CONTENT. Stay off shell. | Owner: Codex quota resolved 2026-07-19. |
