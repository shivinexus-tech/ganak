...
| CODEX-P1-BUNDLE | ACTIVE | Codex | `main` | see `plans/codex-task-phase1-bundle.md` | Tracks 1 content → 2 Chart wire → 3 hide+deploy (sequenced) | Owner assigned 2026-07-19; Track 1 started first. |
| P1-CONTENT | ACTIVE | Codex | `main` | `src/data/festival-meta.ts`, `src/data/vrat-vidhis.ts`, `src/engine/festivals.ts` if required, `validation/content-dates.cjs`, scoped research under `plans/` | Phase 1 content coverage + sourced vrat vidhis | Started 2026-07-19; source-first, no shell edits. |
| SPLIT-UI-CHART-WIRE | RESERVED | Codex | `main` | `src/kundli-app.tsx`, listed Chart/Jyotish peel modules, task log/handoff | Import peeled Chart/Jyotish modules and delete shell copies | Starts only after P1-CONTENT slice; Codex is sole shell writer while ACTIVE. |
| P1-HIDE-DEPLOY | RESERVED | Codex | `main` | minimal shell nav hide, hosting config, deployment docs | Hide Chart entry points and deploy Phase 1 web build | Hard-blocked until SPLIT-UI-CHART-WIRE is MERGED. |
| CLAUDE-DAILY-POLISH | RESERVED | Claude Code | `main` | see `plans/claude-task-daily-prashna-polish.md` | Daily/Prashna/Muhurat polish chips A–E + docs | Owner assigned 2026-07-19. Stay off shell during Codex Chart wire. |
| CHIP-B-I18N | MERGED | Claude Code | `main` | `src/i18n.ts`, `src/screens/MuhuratHub.tsx` | E-0.7 muhurat window labels bilingual | **Root cause was `tr()` itself** — it tested `L["hi"]` (a top-level key that never exists) so every `tr()` string was English-only in Hindi mode; all 38 dictionary entries already had Hindi that was unreachable. One-line fix + 6 new keys + 11 hardcoded call sites converted. Gates green; browser: Devanagari runs 248 → 290, all 6 window labels translate, English unchanged, 0 console errors. |
