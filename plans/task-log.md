...
| CLAUDE-P1-VRATVIDHI-VERIFY | REVIEW | Codex (owner reassigned) | `main` | `src/data/vrat-vidhis.ts`, `src/data/festival-meta.ts`, `src/engine/festivals.ts`, `src/screens/MuhuratHub.tsx`, `validation/content-dates.cjs`, `plans/claude-task-p1-vratvidhi-verify.md`, `plans/p1-vratvidhi-owner-queue.md`, `plans/religious-content-policy.md`, `plans/task-log.md` | Verify and correct wired vidhis | **2026-07-20.** Four Chhath days connected; Chaitra and Sharad separated in calendar, display metadata and full guides; generic hidden `navratri` key removed; Hartalika states traditional nirjala with a separate health note. Parse ×5, parity EXACT, Prashna 24/24, muhurat, content **49/49**, Panchaka regression and build pass. Browser: Hartalika summary/details EN, 0 errors. |
