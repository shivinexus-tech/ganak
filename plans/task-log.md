...
| SPLIT-UI-03-WIRE | BLOCKED | Claude Code | `main` | `src/kundli-app.tsx` (integration-owned) | Wire Cursor's peeled Daily modules into the shell per `plans/cursor-handoff-daily-ready.md` | **Blocked on write contention**, not on code. Another agent was committing every 30–90s at 16:25–16:32 and one such commit already broke `main` once. Needs a quiet window on `src/kundli-app.tsx` before the wire starts. |
...
| FIX-PANCHAKA-01 | DONE (pushed, unmerged) | Claude Code | `claude/interesting-mahavira-ba1576` | `src/engine/panchaka.ts`, `validation/panchaka-windows.cjs` (new) | Suppress sub-minute panchaka windows that rendered start==end ("अग्नि 9:31 PM–9:31 PM", 30 Jul 2026 Delhi) | Root cause: tithi rolled 16→17 at 21:31:08, lagna 10→11 at 21:31:44 — two independent boundaries 36s apart, so the sliver was real, not a stepping artefact. Fix snaps near-coincident boundaries at the *segment* level so lagnaSchedule + panchakaWindows both clean. All 7 gates green + build; new gate 3,730 windows / 135 days × 3 places; browser verified on the reported day, windows still tile sunrise→sunrise. Commit `6ef291e`. |

| CONTENT-CHHATH-01 | DONE | Claude Code | `claude/interesting-mahavira-ba1576` | `src/engine/festivals.ts`, `src/data/festival-meta.ts`, `src/data/vrat-vidhis.ts`, `validation/content-dates.cjs` | Wire Chhath into the calendar — 4 keys (Nahay Khay/Kharna/Sandhya Arghya/Usha Arghya) + a `sandhya` (sunset) kala. Vidhi content already existed; only the calendar keys were missing (TODO at `vrat-vidhis.ts:442`) | **Does NOT touch `src/kundli-app.tsx`** — vidhi reaches the UI by aliasing the four keys onto the existing `chhath` object in the data file, so no shell edit and no contention with the Daily wire. Only day 3 is resolved from the panchang (Kartika Shukla Shashthi at sunset = Drik's "Chhath Puja" date); days 1/2/4 are civil-day offsets, so tithi kshaya can never drop Kharna. All 7 gates + build green; content-dates now 21/21 with 4 new anchors. Dates verified 2025–28 (2025-10-27, 2026-11-15, 2027-11-04, 2028-10-23 Sandhya Arghya), always 4 consecutive days. Browser: all 4 appear in search + full-year list in order, date-picker marks all 4 (Nov 12 unmarked), 0 console errors; vidhi alias resolves for all 4 keys (4 steps, hi+en); bilingual names verified. Reserved by owner instruction 2026-07-20. |
