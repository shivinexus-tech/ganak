...
| `GDF-056` | `PATTERN` · `ACTIVE` | The original textile/card source is a design vocabulary, not one universal floral asset. It contains powder-blue lattice, fine antique-gold framing, crimson punctuation, leaf-vine/floral transitions, ribbons, niches and other elements. The owner rejected treating `485:51` as the whole vocabulary or pasting it into unrelated screens. Owner, 2026-08-19. | Recover existing sources before generating or composing again. Archive `682:22` restores the twelve historical treatments from Figma history and retained exact assets; it is `NOT APPROVED` reference material. Preserve approved screen architecture and choose ornament only for the screen's job. `485:51` remains Festival/Vrat hero-only unless the owner explicitly approves another use. | Website ornament system · integrator + all lanes |
