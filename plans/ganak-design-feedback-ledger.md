...
| `GDF-061` | `GLOBAL` · `CHANGES REQUESTED · RE-QA PENDING` | Owner visual-review cadence requires exactly twelve complete desktop-English primary screens: four Today/Panchang, four Festivals/Vrat and four Muhurat/Prashna. Responsive/HI are adjacent QA evidence, not extra owner decisions. A previous PASS was withdrawn after the exact screenshot-crop correction in `GDF-063`. | Section `769:17254` remains the exact twelve-screen checkpoint but is not owner-review authority until a fresh exact-node audit passes `GDF-012`, `GDF-062` and `GDF-063`. Production routes and behavior remain separately gated. | Batch 01 owner-review workflow · integrator + QA |
